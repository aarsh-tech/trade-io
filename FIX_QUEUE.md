...
- [x] F18 Design tokens + dark mode + `lib/format.ts` (U6, U9)
- [x] F19 App shell status bar: feed, session, token expiry, kill switch, day P&L (U4)
...
- 2026-09-26 F18: semantic tokens in globals.css (`--surface`, `--profit`, `--loss`, `--warn`, `--info` → `bg-surface`, `text-profit`, …; profit/loss text tones are AA-safe on white), a real `.dark` palette, and dark re-pointing of the accent scales (`*-50/100/200` become translucent tints, `*-700/800/900` lighten) so existing emerald/rose/blue/amber utilities stay legible without per-class `dark:`. `next-themes` ThemeProvider (class, system default) in providers, `ThemeToggle` in the top bar, Sonner follows the theme (`ThemedToaster`). Mechanical rewrite of neutral hard-coded classes across 41 tsx files: `bg-white`→`bg-card`, `bg-slate-50/100/200`→`bg-muted…/bg-border`, `border/divide-slate-100..300`→`border-border`, `text-slate-900/800`→`text-foreground`, `700/600`→`text-foreground/75`, `500/400`→`text-muted-foreground` (400/500 left alone in rms-safety-card, live-screener, strategies/[id], data-table, which contain dark panels). New `lib/format.ts` (`formatINR`, `formatPrice`, `formatPct`, `formatQty`, `formatCompactINR`, `pnlClass`, `.num` tabular class); 47 `₹…toLocaleString` sites converted (negatives now render `-₹1,234.00`). tsc passes; NOT eyeballed in a browser. Left over: 24 `toLocaleString` calls (dates, template strings, non-₹) and the old `formatCurrency/formatNumber` in utils.ts still exist; `text-emerald-500/rose-500` P&L colours not yet switched to `text-profit/loss` and no icon-plus-colour pass (F23 a11y); dark-panel files (rms-safety-card, live-screener, strategies/[id], QuickTradePanel) need a visual check in both themes.
- 2026-09-26 F19: new `GET /market/session` (getMarketSessionInfo in market-calendar.ts: pre-open/open/closed/holiday/weekend, next open, close). `components/layout/status-bar.tsx` sits under the top bar: feed state (Live/Delayed/Disconnected/Feed closed from useMarketStore + socket flag) with last-tick age, market session with next open in IST, Kite token countdown (warns under 1 h, expired state) with Connect/Re-login opening BrokerSessionModal, kill-switch alert, and day P&L (risk status totalDailyPnl, realised + unrealised). New shared `useRiskStatus` hook (60 s poll) replaces the top bar own polling; the hard-coded "Live" badge was removed from the top bar. tsc passes on both apps; NOT checked in a browser. Left over: per-price staleness cue in tables, sidebar collapse, and day P&L uses the risk-service approximation (fills-based P&L is the ledger from F16).
- 2026-09-26 F19 follow-up: status-bar day P&L is now fills-based: new `GET /orders/day-pnl` (OrdersService.getDayRealizedPnl, same FIFO matching, charges and IST bucketing as the ledger) + live unrealised from the risk status, so it agrees with the ledger; the RMS kill-switch loss check still uses the risk service approximation (RiskService.getRiskStatus), which should be moved onto the same source. Added `PriceFreshness` (clock icon + tooltip when the socket is down, the feed is delayed, or a symbol has no live tick; silent when the market is closed) on the positions page (table + mobile cards) and LiveAlgoPositionsCard; dashboard movers and portfolio not covered. Sidebar collapse already existed; it is now persisted (`algo-trade-ui`) and the toggle has an aria-label. tsc passes on both apps; not checked in a browser.
