...
export default function MonthlyLedgerPage() {
  ...
  return (
    <div className="space-y-6 animate-[fade-up_0.3s_ease_both] pb-16 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
              ...
              Monthly P&L Ledger & Trade Journal
            </h1>
        ...
        <div className="flex items-center flex-wrap gap-2.5">
          ...
          <div className="flex items-center bg-card border border-border rounded-lg p-1 shadow-2xs">
            <Button
              variant="ghost"
              size="icon"
            ...
            <div className="flex items-center px-2 gap-1.5 border-x border-border">
              <select
                value={selectedMonth}
                onChange={(e) => {
          ...
          >
            <RefreshCcw className={cn("h-3.5 w-3.5", syncMutation.isPending && "animate-spin text-emerald-500")} />
            {syncMutation.isPending ? "Syncing..." : "Sync Broker"}
          </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-border bg-card/60 backdrop-blur shadow-2xs">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <div className="h-6 w-6 rounded-md bg-blue-500/15 text-blue-600 flex items-center justify-center">
              <Award className="h-3.5 w-3.5" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black font-mono text-foreground flex items-baseline gap-2">
        ...
        <Card className="border-border bg-card/60 backdrop-blur shadow-2xs">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              Trading Sessions Result
            </CardTitle>
            <div className="h-6 w-6 rounded-md bg-purple-500/15 text-purple-600 flex items-center justify-center">
          ...
          <CardContent>
            <div className="text-3xl font-black font-mono text-foreground flex items-baseline gap-2">
              ...
              <span className="text-xs font-normal text-muted-foreground">({summary.tradingDaysCount} Active Days)</span>
            </div>
        ...
        <Card className="border-border bg-card/60 backdrop-blur shadow-2xs">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <div className="h-6 w-6 rounded-md bg-amber-500/15 text-amber-600 flex items-center justify-center">
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground mt-2 flex items-center justify-between border-t border-border pt-2">
      ...
      <Card className="border-border bg-card shadow-xs overflow-hidden">
        <CardHeader className="py-4 px-6 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-muted/20">
          ...
          <div className="flex items-center gap-2">
            {selectedDateFilter && (
              <Badge variant="outline" className="text-xs bg-blue-500/10 text-blue-600 border-blue-500/20 gap-1.5 py-1">
                ...
                <button onClick={() => setSelectedDateFilter(null)} className="ml-1 text-muted-foreground hover:text-foreground">
                  ✕
                </button>
              </Badge>
            )}
            {selectedDateFilter && (
              <Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => setSelectedDateFilter(null)}>
                Clear Date Filter
              </Button>
            )}
          </div>
        ...
        <CardContent className="p-6">
          {/* Day of Week Headers */}
          <div className="grid grid-cols-7 gap-2.5 text-center text-xs font-bold text-muted-foreground mb-3">
            ...
            {calendarDays.map((cell, idx) => {
              if (!cell.isCurrentMonth) {
                return (
                  <div
                    key={`empty-${idx}`}
                    className="h-20 sm:h-24 rounded-xl bg-muted/10 border border-border/20 opacity-30"
                  />
                );
              }
              ...
              return (
                <div
                  ...
                  className={cn(
                    "h-20 sm:h-24 p-2.5 rounded-xl border flex flex-col justify-between transition-all select-none relative group",
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      ...
                      {cell.isToday && (
                        <span className="text-[8.5px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-500/15 px-1 py-0.2 rounded">
                          Today
                        </span>
                      )}
                    </div>
                    {hasData && (
                      <span className={cn(
                        ...
                        {cell.item?.tradesCount} {cell.item?.tradesCount === 1 ? "trade" : "trades"}
                      </span>
                    )}
                  </div>
                  ...
                  {hasData ? (
                    <div>
                      <div className={cn(
                        "text-xs sm:text-sm font-bold font-mono text-right",
                      ...
                      <div className="text-[9.5px] text-muted-foreground text-right mt-0.5">
                        Win Rate: {cell.item?.winRate}%
                      </div>
                    </div>
                  ) : (
                    <div className="text-[10px] text-muted-foreground/40 text-center font-medium">
                      No Trades
                    </div>
                  )}
                </div>
      ...
      <Card className="border-border bg-card shadow-xs">
        <CardHeader className="border-b border-border py-4 px-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center flex-wrap gap-2.5">
            {/* Segment Filter */}
            <div className="flex items-center gap-1 bg-muted/80 dark:bg-muted/40 p-1 rounded-xl border border-border shadow-2xs">
              {(
                [
                  { id: "ALL", label: "All Segments", count: segmentCounts.all },
                  { id: "EQUITY", label: "Equity (NSE)", count: segmentCounts.equity },
                  { id: "FNO", label: "F&O (NFO)", count: segmentCounts.fno },
                ] as const
              ).map((seg) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "px-3 py-1.5 rounded-lg text-xs font-semibold transition-all inline-flex items-center gap-1.5 select-none",
                      isActive
            ...
            <div className="relative w-48">
              ...
              <input
                ...
                className="w-full bg-background border border-border text-foreground text-xs rounded-lg pl-8 pr-7 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-2xs placeholder:text-muted-foreground/60"
              />
              {searchSymbol && (
                <button
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">Calculating closed trade ledger from broker records...</p>
            </div>
            ...
            <div className="py-20 text-center px-4">
              ...
              <h3 className="text-base font-semibold text-foreground">No Closed Trades Found</h3>
