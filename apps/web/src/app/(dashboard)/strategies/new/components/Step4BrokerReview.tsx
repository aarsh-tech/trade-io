...
export function Step4BrokerReview({ form, set, brokers }: Step4Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="text-xs sm:text-sm font-extrabold mb-2 block text-foreground">Execution Mode</label>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button
            type="button"
            onClick={() => set("isPaperTrade", true)}
            className={cn(
              "p-4 rounded-2xl border-2 text-left transition-all cursor-pointer bg-card",
              form.isPaperTrade
          ...
          >
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <p className="font-extrabold text-sm sm:text-base text-foreground">Paper Trading (Simulation)</p>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <p className="font-extrabold text-sm sm:text-base text-foreground">Live Broker Execution</p>
            </div>
            <p className="text-xs text-foreground leading-relaxed font-medium">
      ...
      <div>
        <label className="text-xs sm:text-sm font-extrabold mb-2 block text-foreground">Select Broker Account</label>
        {brokers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {brokers.map((b) => (
              <button
                ...
                className={cn(
                  "p-4 rounded-2xl border-2 text-left transition-all flex items-center justify-between cursor-pointer bg-card",
              ...
              >
                <div>
                  <p className="font-extrabold text-sm uppercase text-foreground">{b.broker}</p>
                  <p className="text-xs text-foreground/75 font-bold mt-0.5">
      ...
      <div className="p-5 rounded-2xl border-2 border-border bg-card shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-xs">
          <div>
            <span className="text-xs font-bold text-foreground/75 block mb-0.5">Strategy Name</span>
            <span className="font-black text-sm sm:text-base text-foreground">{form.name}</span>
          </div>
          <div>
            <span className="text-xs font-bold text-foreground/75 block mb-0.5">Type</span>
            <Badge variant="outline" className="text-[10px] font-black border-2 border-border text-foreground mt-0.5">
              {form.type}
            </Badge>
          </div>
          <div>
            <span className="text-xs font-bold text-foreground/75 block mb-0.5">Underlying / Mode</span>
            <span className="font-black text-sm text-foreground">{form.symbol} ({form.product})</span>
          </div>
          <div>
            <span className="text-xs font-bold text-foreground/75 block mb-0.5">Position Size</span>
            <span className="font-black text-sm text-foreground">{form.lots} Lot ({totalQty} Qty)</span>
          </div>
        </div>
