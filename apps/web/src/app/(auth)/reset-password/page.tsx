  ...
  if (!token) {
    return (
      <div className="text-center py-4 space-y-4">
        <h2 className="text-base font-medium text-foreground">Invalid link</h2>
        <p className="text-xs text-muted-foreground">
          This password reset link is invalid or has expired.
        </p>
        <Link href="/forgot-password" className="block pt-2">
          <Button className="w-full bg-primary hover:bg-brand-hover text-primary-foreground text-xs font-medium rounded-[3px] h-[40px]">
            Request new link
          </Button>
        </Link>
      </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="relative">
        <input
          ...
          required
          className="w-full h-[46px] px-3.5 text-sm bg-card text-foreground border border-input rounded-[3px] focus:outline-none focus:border-primary transition-colors placeholder:text-muted-foreground"
        />
        {form.confirm && form.password !== form.confirm && (
          <p className="text-xs text-loss font-normal pt-1">Passwords do not match</p>
        )}
      </div>
      ...
      <div className="pt-2 text-center">
        <Link
          href="/login"
          className="inline-flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-accent-foreground transition-colors"
        >
...
export default function ResetPasswordPage() {
  return (
    ...
    >
      <Suspense
        fallback={<div className="text-center py-6 text-xs text-muted-foreground">Loading...</div>}
      >
        <ResetPasswordForm />
      </Suspense>
    </AuthLayout>
