 ...
 * than discovered mid-session. Mirrors apps/auth-service/src/strategy/strategy-config.validator.ts (keep them in sync); the server is the source of truth.
 *
 * `null` counts as "not set": the wizard serialises an empty numeric input as null (JSON.stringify(NaN)).
 */

export type StrategyCfg = Record<string, unknown>;
type Cfg = StrategyCfg;
